...
# ================= CONFIGURATION =================
# We point to the "resolve" URL of your Hugging Face file
...
# ================= LAZY DATA LOADER =================
# We DO NOT download the file here. We just connect to it.
@st.cache_resource
def get_lazy_frame():
...
# --- FILTER 1: LINES ---
# Hardcoded for speed (scanning 5GB for unique values takes too long)
# You can add more lines here if you want
lines = ["IC 1", "IC 5", "IC 3", "IC 6", "IC 8", "IC 61", "IR 15", "IR 90"]
selected_lines = st.sidebar.multiselect("Select Lines", lines, default=["IC 1", "IC 5"])
...
# 1. Filter by Line (Drastically reduces size)
query = lf.filter(pl.col("LINIEN_TEXT").is_in(selected_lines))

# 2. Collect into RAM (This downloads only the selected lines!)
try:
    df = query.collect()
except Exception as e:
    st.error(f"Error streaming data from Hugging Face: {e}")
    st.stop()

# 3. Post-Processing (Now that it's in RAM, we can add columns fast)
df = df.with_columns([
...
# 4. Apply Neutralization Filter (In Memory)
if neutralize:
    df = df.filter(
...
st.markdown(f"Analysis of **{df.height:,}** intercity journeys.")
